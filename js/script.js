function getBooks(bookTitle) {
  var from = rxjs.from;
  var _a = rxjs.operators,
    switchMap = _a.switchMap,
    map = _a.map,
    tap = _a.tap; // tap per mostrare dati che ci stanno arrivando
  var apiUrl = "https://www.googleapis.com/books/v1/volumes?q=";

  /*  var apiUrl = "https://www.googleapis.com/books/v1/volumes?q="; */
  /* chiamata fetch all'api di google */
  var p = fetch(apiUrl + bookTitle).then(function (res) {
    return res.json();
  }); // promise con api fetch
  ...
  from(p)
    .pipe(
      tap(function (data) {
        return showTotal(data.items.length);
      }),
      switchMap(function (data) {
        return from(data.items || []);
      }), // trasformare gli items in una string ritornando gli elementi uno ad uno ||o array vuoto per non avere l'errore in caso non ci siano risultati per l'input di ricerca
      map(function (ele) {
...
function displayBook(book) {
  // mostrre a display
  var bookTpl =
    /*<div class="col"> commentato per fare append child */ '\n    <div class="card shadow-sm">\n        <img src="' +
    book.thumbnail +
    '" title="' +
    book.title +
    '" alt="' +
  ...
  if (searchEle) {
    fromEvent(searchEle, "keyup")
      .pipe(
        ...
        filter(function (ele) {
          return ele.length > 2;
        }), //per drezionare questa stinga a getBooks
        debounceTime(1000), // tempo di attesa prima del risultato del subscribe
        tap(function () {
          return cleanBooks();
        }), // funz che pulisce Dom non modifica la string
        switchMap(function (ele) {
          return getBooks(ele);
        }) //ridezionare questa stringa all'altro observable getbooks
      )
      .subscribe(function (book) {
        return displayBook(book);
      });
